...
function wordsList(str, subStr) {
    ...
    for (var i in strArray) {
        ...
        if (strArray[i].indexOf(subStr) !== -1) set.add(strArray[i]);
    }
    return set;
}
...
Car = function (engine, model, name, year) {
    ...
    this.year = year;
    var date = new Date();
    if (date.getFullYear() === year) this._used = 'new';
    else this._used = 'used';
    return this;
};
...
Car.prototype = {
    set used(str) {
        ...
        if(str === 'used' && this.year != new Date().getFullYear()) this._used = str;
    },
    get used() {
        return this._used;
    },
